...
/n1:n1-queue --story STORY-12       # run story subtasks through the pipeline one by one without merging. Also: --tag <tag>, --dry-run, --status, --watch.
```
...
| /n1:n1-queue | Run a batch of tracker tickets (by tag or story subtasks) through the pipeline one after another without merging. `--tag`, `--story`, `--dry-run`, `--status`, `--watch`. |
