...
5. `logging` → **Logging Configuration** (fresh-setup portion)
6. `estimation` → **Estimation Configuration** (fresh-setup portion)
7. `localTesting` → **Local Testing Configuration** (fresh-setup portion)
8. `finishWork` → **Finish Work Configuration** (fresh-setup portion)
9. `release` → **Release Configuration** (fresh-setup portion)
10. `codex` → **Codex Review Configuration** (fresh-setup portion)
11. `testCoverage` → **Test Coverage Configuration** (fresh-setup portion)
12. `telemetry` → **Telemetry Configuration** (fresh-setup portion)
13. `analysisCache` → **Analysis Cache Configuration** (fresh-setup portion)
14. `rules` → **Rules Configuration** (fresh-setup portion)
15. `worktree` → **Worktree Setup Detection** (silent detection, no prompt)
16. `escalation` → **Escalation Channel Configuration** (asks channel question when tracker is configured; writes defaults silently when no tracker)
17. `autonomy` → **Autonomy Configuration** (fresh-setup portion: offer preset selection)
18. `review`, `ciChecks`, `planReview`, `memory`, `models` → write defaults silently (see **Write Configuration and Structure** for default values)
