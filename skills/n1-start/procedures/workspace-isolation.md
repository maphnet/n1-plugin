...
1. Compute target from `git.branchPattern`+`<ID>`. Sanitize (lowercase, illegal→`-`). 2. `CURRENT=$(git branch --show-current)`, `DEFAULT`, `DIRTY`. 3. `CURRENT==TARGET`→reuse. Target exists+clean→checkout. `CURRENT==DEFAULT`+clean→`git checkout -b <TARGET>`. Dirty/foreign: `MP=$(n1_autonomy_val 'mechanicalPrompts')`. `MP=auto`: stash/switch as needed+Decision Ledger row. `MP=ask`: `procedures/workspace-isolation-recovery.md`. 4. `[ -f "$BP_FILE" ] || git rev-parse HEAD > "$N1_HOME/memory/<ID>/branch-point"`. 5. Report.

## Ensure Worktree (`<ID>`)

1. `N1_HOME` must be absolute; relative→error "Run `/n1:n1-init` or use `--branch`." STOP. 2. Compute target. Check `git worktree list --porcelain`: exists→resume. 3. Not exists:
```bash
...
```
`git worktree add` fails→`procedures/workspace-isolation-recovery.md`.
...
`oldId==newId`→return. Memory: `mv <oldId>/ <newId>/`. Frontmatter: rewrite `ticket:`. Branch: `git branch -m`. Worktree: `git worktree move`. Update active-run:
```bash
N1_ROOT="${CLAUDE_PLUGIN_ROOT}"; [ -d "$N1_ROOT/lib" ] || N1_ROOT=$(python3 -c 'import json,os;print(json.load(open(os.path.expanduser("~/.n1/host.json")))["pluginRoot"])')
source "$N1_ROOT/lib/step.sh"; source "$N1_ROOT/lib/config.sh"
...
`USE_WORKTREE=true`+`WORKTREE_PATH` set: pass to every agent touching source: "Work in worktree `$WORKTREE_PATH`. All file ops and git/bash MUST target files within. Memory: `$N1_HOME/memory/<ID>/`." Unexpected→`procedures/workspace-isolation-recovery.md`.
