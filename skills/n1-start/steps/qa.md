...
**Spawn qa-engineer** (context `qa`, tier from `n1_config_val '.testCoverage.tier'` default `maintain`). Inputs: ticket.md, implementation.md, plan.md or brainstorm.md; Key Decisions+Escalations inline; `$RULES_BLOCK`. Output: write `qa.md`; return `Verdict: PASS|FAIL` / `Bugs found: yes|no` / `TQ-relevant notes:` / summary / `n1:signals`.
...
if [ "${VERIFY_GATE}" = "true" ]; then
    ...
    if [ -n "$RUNNER_CMD" ]; then
        ...
        if [ "$ACTUAL_EXIT" != "$REPORTED_EXIT" ]; then
            n1_append_key_decision "$N1_HOME/memory/$ID/overview.md" "QA verifyGate mismatch: reported ${REPORTED_EXIT}, actual ${ACTUAL_EXIT}. Log: $VERIFY_LOG"
            n1_write_frontmatter "$N1_HOME/memory/$ID/overview.md" "qa_verdict_unverified" "true"; QA_DEGRADED=1
        fi
    else n1_append_key_decision "$N1_HOME/memory/$ID/overview.md" "QA verifyGate skipped: no 'Runner command:' in qa.md"; fi
fi
```
`NEW_FUNC_UNTESTED=true`: tier-B Decision Ledger row. `BLOCK_UNTESTED=true`: override verdict FAIL. `QA_DEGRADED=1`: print `⚠ QA evidence missing`.
...
grep '^New test:' "$N1_HOME/memory/$ID/qa.md" | head -n "$BC_MAX" | while IFS= read -r line; do
    ...
    echo "$NAME $(echo "$J" | jq -r '.verdict // "inconclusive"')"
...
```
Append `## Break-check` to qa.md. Bug: `BC_VERDICT != red-then-green` → `QA_RESULT=FAIL`, fix cycle: "Regression `<REG_NAME>` does not fail on revert." Non-bug types: `BC_MODE==all` blocking; else pass `BREAK_CHECK_TQ` to review. Update overview: `[x] QA`, `step: qa`. Maintain+PASS+"No test work needed" → skip fix loop.

**FAIL:** spawn developer with bug details + "Record under `## QA Fix Cycle <N>` in implementation.md."

Bounded `qa.maxFixAttempts` (default 3) → § Autonomy Gate. **Headless:** `procedures/autonomy-headless.md § Headless Guard`. **If ask:** details + advise.
