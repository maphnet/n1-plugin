...
**Simplicity gate PASS** (all: `TIER==simple`, `BLAST==low`, `FILES_CHANGED<3`): dispatch **developer** persona with `$DEVELOPER_MODEL` and `$DEVELOPER_EFFORT`, brainstorm.md or plan.md, and "Direct Implementation mode." → QA.

**ANY fails:** `PLANNING_NEED=direct` → dispatch **developer** persona with the resolved model/effort pair, brainstorm.md, "Direct Implementation." `PLANNING_NEED=plan`/absent → plan.md; absent → Plan path. Top-level headers >2 → Plan path; ≤2 no cross-deps → dispatch developer with the resolved pair, plan.md, "Direct, sequential"; else Plan path.
