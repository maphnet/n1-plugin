...
- **PR review** (Step 7 / `n1-review` Phase 2): `n1_codex_preflight "<BASE_BRANCH>"` check (availability + branch resolution), then `codex-reviewer` agent (spawned via Agent tool) runs the Codex CLI, retries on failure, and parses output into `[CX-N]`-prefixed structured findings merged into `review.md` alongside `[CR-N]` and `[SEC-N]`.
