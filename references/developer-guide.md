...
`python3 scripts/telemetry_analyzer.py` is the run-performance analyzer behind `/n1:n1-telemetry-analyzer`. `collect` loads merged run records (reusing benchmark.py's `load_runs`/`read_jsonl`), extracts per-run metrics (step durations, agent counts, token totals, tool call counts), detects anomalies (slow steps, token-heavy agents, low cache efficiency, excessive fix cycles), and computes cross-run aggregations (avg/median/P90/min/max) by tier and step. Flags: `--last N` (analyze the N most recent runs, default 10), `--projects p1,p2` (filter by project slug), `--deep` (enrich with transcript data), `--out <file>` (write JSON to file instead of stdout). The skill drives the script, formats its JSON output as Markdown summary tables, aggregation tables, anomaly list, and per-run detail breakdowns.
