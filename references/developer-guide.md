...
- **Shared shell helpers:** `lib/preamble.sh` (canonical bash preamble — sources `lib/config.sh` and helpers, resolves `N1_ROOT`/`N1_HOME`; sourced at the top of every bash snippet in skills), `lib/host.sh` (host detection, plugin root, headless command), `lib/agent_profiles.py` (Codex persona TOML generator), `lib/transcript_codex.py` (Codex rollout parser), `lib/config.sh` (codex/model resolution), `lib/signals.sh` (signal read/write/gate evaluation), `lib/step.sh` (per-step begin/end helpers: telemetry, frontmatter, signal persistence, decision records), `lib/context.sh` (context-persistence: write/read TIER/TYPE/DESC_QUALITY/LITE_MODE to `ticket-context.sh` so downstream bash snippets can source it instead of re-deriving from frontmatter), `lib/memory.sh` (compaction), `lib/cache.sh` (analysis snapshot I/O and freshness check), `lib/rules.sh` (rules directory resolution, file parsing, agent filtering, injection rendering, deny hook generation), `lib/queue.sh` (queue runner + story-mode helpers: service→repo lookup, model pick, child status/launch)
...
`python3 scripts/benchmark.py` is the orchestrator benchmark behind `/n1:n1-benchmark`. `collect` scans all run records under `~/.n1/*/memory/*/telemetry/runs/`, links each completed run to its Claude Code session transcript (via the raw agents file, falling back to project slug plus time window plus ticket ID), extracts human turns, and classifies them heuristically; ambiguous turns are labeled by a Haiku judge in the skill and passed back via `finalize --labels`, which computes metrics (including orchestrator API call counts, bash call counts, and input tokens) and writes a snapshot. `report` renders per-version tables with bootstrap confidence intervals and deltas against the previous snapshot and the pinned baseline (`baseline set <version>`), plus a gate activation rates table (gates with <5% activation across collected runs are flagged as removal candidates). State lives in `~/.n1/benchmark/`. Tests: `bash tests/test_benchmark.sh` (integration) and `python3 -m pytest tests/test_benchmark.py` (unit — covers metric classes; not run by `run.sh`).

`python3 scripts/telemetry_analyzer.py` is the run-performance analyzer behind `/n1:n1-telemetry-analyzer`. Subcommands: `collect` (per-run metrics and anomalies), `compare <run-id-a> <run-id-b>` (differential report showing metric deltas between two runs). `collect` loads merged run records (reusing benchmark.py's `load_runs`/`read_jsonl`), extracts per-run metrics (step durations, agent counts, token totals, tool call counts), detects anomalies (slow steps, token-heavy agents, low cache efficiency, excessive fix cycles), and computes cross-run aggregations (avg/median/P90/min/max) by tier and step. Flags: `--last N` (analyze the N most recent runs, default 10), `--projects p1,p2` (filter by project slug), `--deep` (enrich with transcript data), `--attribution` (break down token usage by step category with unattributed overhead), `--out <file>` (write JSON to file instead of stdout). The skill drives the script, formats its JSON output as Markdown summary tables, aggregation tables, anomaly list, and per-run detail breakdowns.
