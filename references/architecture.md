...
**Same-ticket busy guard:** resuming an in-progress ticket calls `n1_busy_guard` (`lib/frontmatter.sh`) against overview.md's `pid` field, mirroring the queue runner's busy guard (see `scripts/n1-queue-run.sh <queue.md>` above). No pid, a dead pid, or a live pid equal to the current process (post-compaction resume) all proceed silently and (re)write the current pid; a live pid belonging to a different process warns and continues interactively, or exits 3 under `N1_HEADLESS=1`.
...
- **Merge:** `mergeOnFinish` (default `false`, reviewer merges; takes effect only with `finishWork.enabled: true`, decided by `n1_merge_allowed`) triggers `gh pr merge --auto --<mergeMethod> --delete-branch` when enabled.
...
`scripts/n1-queue-run.sh <queue.md>` is a pure-bash sequential batch executor. The queue.md file has YAML frontmatter (`run_id`, `pid`, `step`, `queue_id`) and a `## Plan` table with columns `# | Ticket | Title | Repo | N1 Home | Model | Status | Reason`. Statuses: `pending`, `in-progress`, `awaiting-human`, `pr`, `escalated`, `failed`, `deferred`, `skip`, `done-before-run`. Each child runs under `N1_HEADLESS=1 N1_AUTONOMY_PRESET=autonomous N1_STOP_AT=ci` with `N1_QUEUE_RUN_ID` set to the run identifier. Children merge only when `queue.mergeOnFinish` is `true` (default `false`, independent of `finishWork.mergeOnFinish`): `n1_finish_enabled`/`n1_merge_allowed` gate the n1-start finish step, n1-ci chaining and n1-finish, and `hooks/enforce-agent-policy.py` Case 3 denies `gh pr merge`, `gh api …/merge` and `git merge`/`git push` to the default branch at PreToolUse on both hosts. Defer-once: a first failure marks the row `deferred` and appends a new `pending` row with reason `deferred-retry`; a second failure stays `failed`. Three-strikes: 3 consecutive non-success outcomes (`failed`/`escalated`) halt the queue (`step: halted`, exit 2); a `pr` outcome resets the counter. Busy guard: exits 3 if frontmatter `pid` is alive. Helpers: `lib/queue.sh`; defaults: `defaults/queue.json`. The `--status` report writes `<queue-dir>/telemetry.json` with three counts derived by `n1_queue_decision_counts`: `plan_decisions` (Decision Ledger `preview|edit` rows), `autonomous_decisions` (sum of `headless` ledger rows across all non-pending/skip/done-before-run ticket overviews), and `escalations` (Plan rows with status `escalated`). On handoff (PR, escalation, failed) the queue tag is released via `skills/n1-queue/procedures/release-tag.md` (best-effort, `editTicket`); the runner stamps `queue_run_id` into the ticket's `overview.md`, and intake excludes a tagged ticket until `queue_tag_removed: true` confirms the release, so a stale tag never re-picks a ticket while a human re-tag re-queues it.
