...
`~/.n1/preamble.sh` is a two-line shim generated by `hooks/session-start.sh` next to `~/.n1/host.json` on every startup, resume, clear and compact, on both hosts (last session start wins, as with `host.json`); it is written, not symlinked, so it works in Git Bash/MSYS without native symlink support. It sets `N1_ROOT` to the plugin root and sources `lib/preamble.sh`, which sources `lib/config.sh` (which sources `lib/host.sh`), `lib/step.sh` and `lib/validation.sh`, and sets `N1_HOME`. Specialized libs are sourced after it as `source "$N1_ROOT/lib/<lib>.sh"`. Never resolve `N1_ROOT` inline; `tests/test_host_neutral_skills.sh` rejects it.
