...
test_toposort() {
    local out
    out=$(n1_queue_toposort "A,B,C" "" | tr '\n' ',')
    assert_eq "qtopo: no edges" "A,B,C," "$out"
    if n1_queue_toposort "A,B" $'A>B\nB>A' >/dev/null 2>&1; then
        assert_eq "qtopo: cycle exits 2" "2" "0"
    else
        assert_eq "qtopo: cycle exits 2" "2" "$?"
    fi
}
