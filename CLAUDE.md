...
**Skills:** start every bash snippet with `source ~/.n1/preamble.sh`. `~/.n1/preamble.sh` is a one-line constant trampoline generated by `hooks/session-start.sh` on every session start (not a symlink — Git Bash/MSYS has no reliable `ln -s`); it sources `~/.n1/sessions/<sid>.preamble.sh` (sid from the harness env), which sets that session's `N1_ROOT` and sources `lib/preamble.sh`, which sources `lib/config.sh` and other helpers and sets `N1_HOME`. See `references/host-routing.md § Bash snippet preamble`.
