...
- **Workspace isolation lifecycle:** `n1-start` resolves isolation mode via: `--step` > `--branch` flag > `worktree.mode` config > default worktree. In worktree mode (default), it creates a worktree at `<main-checkout>/.claude/worktrees/<ID>/` via `Ensure Worktree`. In branch mode, it creates a feature branch eagerly in Step 1 via `Ensure Working Branch`. `n1-pr` performs `git push -u origin <branch>` and removes the worktree after push when `worktree.cleanup` is `"after-pr"` (skipped entirely when `git.prMode` is `"skip"`); in branch mode the branch is preserved.
