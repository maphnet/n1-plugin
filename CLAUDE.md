...
**Skills:** start every bash snippet with `source ~/.n1/preamble.sh`. `~/.n1/preamble.sh` is a two-line shim generated by `hooks/session-start.sh` on every session start (not a symlink — Git Bash/MSYS has no reliable `ln -s`); it sets `N1_ROOT` and sources `lib/preamble.sh`, which provides `N1_ROOT`, sources `lib/config.sh` and other helpers, and sets `N1_HOME`. See `references/host-routing.md § Bash snippet preamble`.
