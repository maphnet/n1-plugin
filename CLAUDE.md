...
- **PR review** (Step 7 / `n1-review` Phase 2): `n1_codex_available` check, then `node "$CODEX" review --wait --scope branch --base <branch> [--model] --effort <effort|medium>`. `codex-adapter` agent parses output into `[CX-N]`-prefixed structured findings merged into `review.md` alongside `[CR-N]` and `[SEC-N]`.
...
- **Availability gate:** `n1_codex_available` encapsulates the full probe. If the plugin or CLI is absent, the step logs a skip note and proceeds Claude-only — Codex is a soft/optional dependency with no `.claude-plugin/plugin.json` entry.
- **Partial-failure handling:** a failed or timed-out Codex call is retried once, then the review proceeds with the remaining reviewers and records the gap; a missing Codex reviewer is never treated as a PASS.
